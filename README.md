...
由`build`函数构建的组件，在元素树中进行比较，再进行必要的更新，以至渲染树得到必要的渲染

#### 2. UI是如何更新的？

通过检查有状态组件的每一个部分组件，并将改变的部分进行更新。

因此，最理想的情况就是将需要改变状态的组件最小化分离出来，并放入无状态组件中进行使用。

#### 3. 理解Key

`flutter`的渲染机制是不会改变状态的位置，即便组件的位置发生改变，它对应的状态位置仍然是原来的位置，为了解决这个问题要为这个列表当中的每一个元素新增一个`key`值。

使得渲染树不仅检查元素的位置，还要检查元素与状态是否对应

如果不对应，就改变状态的位置

#### 4. 变量的声明

- `final`，元素指向的空间或对象不可更改，但允许新增、删除操作

- `var`，元素指向的空间或对象可更改

- `const`，元素指向的空间或对象不可更改，也不允许新增删除操作，即该变量成为了常量
